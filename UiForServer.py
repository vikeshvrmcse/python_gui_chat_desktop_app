...
# dynamic_background_color="#185519"
# dynamic_text_color="#F5F7F8"
# dynamic_border_color="#FCDE70"
# dynamic_hover_color="#E8B86D"

# root.configure(fg_color=dynamic_background_color)
# root.geometry("900x700")
# root.title("BASE HANDLER")

# heading_label=ctk.CTkLabel(root,text="Welcome, Dear !!!, Server", height=100,font=("New Roman",40,"italic"),fg_color=dynamic_hover_color,corner_radius=15)
...
#     receive_thread = threading.Thread(target=svr.receiveData)
#     send_thread = threading.Thread(target=svr.sendData,args=[send_message])
...
from PIL import Image
import threading
import Server as svr  # Make sure this is a module with receiveData and sendData functions
...
heading_label = ctk.CTkLabel(root, text="Welcome, Dear !!!, Server", height=100, font=("New Roman", 40, "italic"),
...
# Function to create a new label in the scrollable frame based on server response
def createLabel():
...
# Function to receive and update the UI with the server's response
def update_received_message():
    received_data = svr.receiveData()  # Simulating the function that fetches the server response

    # Use the main thread to update the UI safely
    root.after(0, lambda: create_server_label(received_data))


# Helper function to create a server message label
def create_server_label(message):
